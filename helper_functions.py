...
#    Required if not using a p_file. if p_file specifed these paramters will overwrite the values from the file
    ...
    parser.add_argument("-struct", help="structural filename (Required if p_file not used)")
...
def process_cmdln(args,code_ver):
    ...
    key_list=['Hb','PaO20','PLD','echo1_fn','echo2_fn', 'M0_fn','struct_fn']
    ...
    if args.p_file==None:
        ...
        d_analysis['struct_fn']=args.struct   
...
# check all essential keys are in p_file
 ...
 #    read in keys for input files (which should not be in d_analysis dictionary yet)
            d_analysis['echo1_fn']=read_dict['echo1_fn']
            d_analysis['echo2_fn']=read_dict['echo2_fn'] 
            d_analysis['M0_fn']=read_dict['M0_fn'] 
            d_analysis['struct_fn']=read_dict['struct_fn'] 
...
#        to one 3D dataset.
    ...
    try:
        struct_img=nib.load(d_analysis['struct_fn'])
        struct_data=struct_img.get_data()
    except Exception as error:
            print(error)
            raise SystemExit(0)  
    ...
    images_dict['struct_img']=struct_img 
    images_dict['struct_data']=struct_data 
